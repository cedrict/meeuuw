...
def output_solution_to_vtu(
    ...
    # --
    vtufile.write(
        "<DataArray type='Float32' NumberOfComponents='3' Name='Velocity (" + vel_unit + ")' Format='ascii'> \n"
    )
    for i in range(0, nn_V):
        vtufile.write("%.4e %.1e %.4e \n" % (u[i] / vel_scale, 0.0, w[i] / vel_scale))
    vtufile.write("</DataArray>\n")
    # --
    # if debug_sol:
    ...
    if geometry == "quarter" or geometry == "half" or geometry == "eighth":
        ...
        for i in range(0, nn_V):
            vtufile.write("%.3e %.1e %.3e \n" % (vr[i] / vel_scale, 0.0, vt[i] / vel_scale))
        vtufile.write("</DataArray>\n")
    # --
    if solve_Stokes:
    ...
    if solve_Stokes and (exp == 19 or exp == 21 or exp == 22):
        ...
        vtufile.write("</DataArray>\n")
    # --
    if abs(min(plith)-max(plith))>1e-6:
    ...
    # --
    if solve_Stokes:
        if particle_eta_projection == "elemental":
            vtufile.write("<DataArray type='Float32' Name='Viscosity (*)' Format='ascii'> \n")
        else:
            vtufile.write("<DataArray type='Float32' Name='Viscosity' Format='ascii'> \n")
        eta_elemental.tofile(vtufile, sep=" ", format="%.4e")
        vtufile.write("</DataArray>\n")
    # --
