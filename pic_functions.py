...
@numba.njit
def interpolate_field_on_particles(nparticle, swarm_active, swarm_r, swarm_t, swarm_iel, phi, icon):
    """
    Args:
...
def locate_particles___box(
    ...
    Args:
       nparticle: number of particles
       swarm_active(nparticle):
       swarm_x(nn_V),swarm_z(nn_V): cartesian coordinates arrays of all particles
       hx,hz: element size
       x_V(nn_V),z_V(nn_V): cartesian coordinate arrays of V nodes
       icon(m_V,nel): connectivity of V nodes
       nelx: nb of elements in x direction
    Returns:
       swarm_r(nparticle): reduced coordinates r of all particles
       swarm_t(nparticle): reduced coordinates t of all particles
       swarm_iel(nparticle): cell index of all particles
    """
...
@numba.njit
def locate_particles___annulus(
    nparticle, swarm_active, swarm_rad, swarm_theta, hrad, htheta, rad_V, theta_V, icon, nelx, Rinner
):
    """
    Args:
       nparticle: number of particles
...
def advect_particles___box(
    ...
    if RKorder == 1:
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                swarm_u[ip], swarm_w[ip], iel = interpolate_vel_on_pt___box(
                    swarm_x[ip], swarm_z[ip], u, w, hx, hz, nelx, icon, x_V, z_V
                )
...
def advect_particles___eighth(
    ...
    if RKorder == 1:
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                swarm_u[ip], swarm_w[ip], iel = interpolate_vel_on_pt___annulus(
                    swarm_x[ip], swarm_z[ip], u, w, hrad, htheta, nelx, icon, rad_V, theta_V, Rinner
                )
                swarm_x[ip] += swarm_u[ip] * dt
                swarm_z[ip] += swarm_w[ip] * dt
...
@numba.njit
def advect_particles___annulus(
    RKorder,
    dt,
    nparticle,
    swarm_x,
...
@numba.njit
def advect_particles___quarter(
    RKorder,
    dt,
    nparticle,
    swarm_x,
    ...
    if RKorder == 1:
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                xA = swarm_x[ip]
                zA = swarm_z[ip]
                uA, wA, iel = interpolate_vel_on_pt___annulus(
                    xA, zA, u, w, hrad, htheta, nelx, icon, rad_V, theta_V, Rinner
                )
                ...
                if xB < 0 or yB < 0 or rB < Rinner or rB > Router:
                    ...
                    uB, wB, iel = interpolate_vel_on_pt___annulus(
                        xB, yB, u, w, hrad, htheta, nelx, icon, rad_V, theta_V, Rinner
                    )
                    swarm_x[ip] = xA + uB * dt
                    swarm_z[ip] = zA + wB * dt
                    swarm_u[ip] = uB
...
@numba.njit
def project_particles_on_elements(nel, nparticle, swarm_active, swarm_rho, swarm_eta, swarm_iel, averaging):
    """
    Args:
    ...
    nparticle_per_element = np.zeros(nel, dtype=np.int32)
    ...
    if averaging == "arithmetic":
        ...
        eta_elemental = 10.0 ** (eta_elemental / nparticle_per_element)

    elif averaging == "harmonic":
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                eta_elemental[swarm_iel[ip]] += 1.0 / swarm_eta[ip]
...
@numba.njit
def project_particle_field_on_nodes_1(
    nel, nn_V, nparticle, swarm_active, swarm_phi, icon, swarm_iel, swarm_r, swarm_t, averaging
):
    """
    Args:
       nel: number of elements
    ...
    if averaging == "arithmetic":
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                iel = swarm_iel[ip]
                for k in (0, 1, 2, 3):
                    phi_nodal[icon[k, iel]] += 1.0 / swarm_phi[ip]
                    count_nodal[icon[k, iel]] += 1
                # end for
            # end if
        # end for
    ...
    for iel in range(0, nel):
        ...
        phi_nodal[icon[8, iel]] = 0.25 * (
            phi_nodal[icon[0, iel]] + phi_nodal[icon[1, iel]] + phi_nodal[icon[2, iel]] + phi_nodal[icon[3, iel]]
        )
...
@numba.njit
def project_particle_field_on_nodes_2(
    nel, nn_V, nparticle, swarm_active, swarm_phi, icon, swarm_iel, swarm_r, swarm_t, averaging
):
    """
    Args:
    ...
    if averaging == "arithmetic":
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                iel = swarm_iel[ip]
                for k in (0, 1, 2, 3):
                    phi_nodal[icon[k, iel]] += 1.0 / swarm_phi[ip] * N[k]
                    count_nodal[icon[k, iel]] += 1 * N[k]
                # end for
            # end if
        # end for
...
# in the corresponding quadrants of each neighouring element, without Q1 weighing
###################################################################################################


@numba.njit
def project_particle_field_on_nodes_3(
    nel, nn_V, nparticle, swarm_active, swarm_phi, icon, swarm_iel, swarm_r, swarm_t, averaging
):
    """
    Args:
    ...
    if averaging == "arithmetic":
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[0, iel]] += swarm_phi[ip]
                    count_nodal[icon[0, iel]] += 1
                # lower right corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[1, iel]] += swarm_phi[ip]
                    count_nodal[icon[1, iel]] += 1
                # upper right corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[2, iel]] += swarm_phi[ip]
                    count_nodal[icon[2, iel]] += 1
                # upper left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[3, iel]] += swarm_phi[ip]
                    count_nodal[icon[3, iel]] += 1
                # end for
            # end if
        # end for
        phi_nodal /= count_nodal
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                # lower left corner - node 0
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[0, iel]] += np.log10(swarm_phi[ip])
                    count_nodal[icon[0, iel]] += 1
                # lower right corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[1, iel]] += np.log10(swarm_phi[ip])
                    count_nodal[icon[1, iel]] += 1
                # upper right corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[2, iel]] += np.log10(swarm_phi[ip])
                    count_nodal[icon[2, iel]] += 1
                # upper left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[3, iel]] += np.log10(swarm_phi[ip])
                    count_nodal[icon[3, iel]] += 1
                # end for
            # end if
        # end for
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                # lower left corner - node 0
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[0, iel]] += 1.0 / swarm_phi[ip]
                    count_nodal[icon[0, iel]] += 1
                # lower right corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    phi_nodal[icon[1, iel]] += 1.0 / swarm_phi[ip]
                    count_nodal[icon[1, iel]] += 1
                # upper right corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[2, iel]] += 1.0 / swarm_phi[ip]
                    count_nodal[icon[2, iel]] += 1
                # upper left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    phi_nodal[icon[3, iel]] += 1.0 / swarm_phi[ip]
                    count_nodal[icon[3, iel]] += 1
                # end for
            # end if
        # end for
...
###################################################################################################
# this version of the algorithm uses the particles around a given node, but only those
# in the corresponding quadrants of each neighouring element, with Q1 weighing
...
@numba.njit
def project_particle_field_on_nodes_4(
    nel, nn_V, nparticle, swarm_active, swarm_phi, icon, swarm_iel, swarm_r, swarm_t, averaging
):
    """
    Args:
    ...
    if averaging == "arithmetic":
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    ...
                    phi_nodal[icon[0, iel]] += swarm_phi[ip] * N
                    count_nodal[icon[0, iel]] += 1 * N
                # lower left corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    N = (1 - swarm_r[ip]) * -swarm_t[ip]
                    phi_nodal[icon[1, iel]] += swarm_phi[ip] * N
                    count_nodal[icon[1, iel]] += 1 * N
                # lower left corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    N = (1 - swarm_r[ip]) * (1 - swarm_t[ip])
                    phi_nodal[icon[2, iel]] += swarm_phi[ip] * N
                    count_nodal[icon[2, iel]] += 1 * N
                # lower left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    N = -swarm_r[ip] * (1 - swarm_t[ip])
                    phi_nodal[icon[3, iel]] += swarm_phi[ip] * N
                    count_nodal[icon[3, iel]] += 1 * N
                # end for
            # end if
        # end for
        phi_nodal /= count_nodal
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    N = -swarm_r[ip] * -swarm_t[ip]
                    phi_nodal[icon[0, iel]] += np.log10(swarm_phi[ip]) * N
                    count_nodal[icon[0, iel]] += 1 * N
                # lower left corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    N = (1 - swarm_r[ip]) * -swarm_t[ip]
                    phi_nodal[icon[1, iel]] += np.log10(swarm_phi[ip]) * N
                    count_nodal[icon[1, iel]] += 1 * N
                # lower left corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    N = (1 - swarm_r[ip]) * (1 - swarm_t[ip])
                    phi_nodal[icon[2, iel]] += np.log10(swarm_phi[ip]) * N
                    count_nodal[icon[2, iel]] += 1 * N
                # lower left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    N = -swarm_r[ip] * (1 - swarm_t[ip])
                    phi_nodal[icon[3, iel]] += np.log10(swarm_phi[ip]) * N
                    count_nodal[icon[3, iel]] += 1 * N
                # end for
            # end if
        # end for
        ...
        for ip in range(0, nparticle):
            if swarm_active[ip]:
                ...
                if swarm_r[ip] <= 0 and swarm_t[ip] <= 0:
                    N = -swarm_r[ip] * -swarm_t[ip]
                    phi_nodal[icon[0, iel]] += 1.0 / swarm_phi[ip] * N
                    count_nodal[icon[0, iel]] += 1 * N
                # lower left corner - node 1
                if swarm_r[ip] >= 0 and swarm_t[ip] <= 0:
                    N = (1 - swarm_r[ip]) * -swarm_t[ip]
                    phi_nodal[icon[1, iel]] += 1.0 / swarm_phi[ip] * N
                    count_nodal[icon[1, iel]] += 1 * N
                # lower left corner - node 2
                if swarm_r[ip] >= 0 and swarm_t[ip] >= 0:
                    N = (1 - swarm_r[ip]) * (1 - swarm_t[ip])
                    phi_nodal[icon[2, iel]] += 1.0 / swarm_phi[ip] * N
                    count_nodal[icon[2, iel]] += 1 * N
                # lower left corner - node 3
                if swarm_r[ip] <= 0 and swarm_t[ip] >= 0:
                    N = -swarm_r[ip] * (1 - swarm_t[ip])
                    phi_nodal[icon[3, iel]] += 1.0 / swarm_phi[ip] * N
                    count_nodal[icon[3, iel]] += 1 * N
                # end for
            # end if
        # end for
...
def compute_ls_coefficients_P1(nel, x_e, z_e, swarm_x, swarm_z, swarm_iel, swarm_rho, swarm_eta):
    ...
    for iel in range(0, nel):
        ...
        rho_min_e[iel] = np.min(s_rho)
        rho_max_e[iel] = np.max(s_rho)
        eta_min_e[iel] = np.min(s_eta)
        eta_max_e[iel] = np.max(s_eta)
...
def output_fields_ls_P1(istep, nel, x, z, icon, x_e, z_e, ls_rho_a, ls_rho_b, ls_rho_c, ls_eta_a, ls_eta_b, ls_eta_c):
    ...
    for iel in range(0, nel):
        vtufile.write("%.4e %.1e %.4e \n" % (x[icon[0, iel]], 0.0, z[icon[0, iel]]))
        vtufile.write("%.4e %.1e %.4e \n" % (x[icon[1, iel]], 0.0, z[icon[1, iel]]))
        vtufile.write("%.4e %.1e %.4e \n" % (x[icon[2, iel]], 0.0, z[icon[2, iel]]))
        vtufile.write("%.4e %.1e %.4e \n" % (x[icon[3, iel]], 0.0, z[icon[3, iel]]))
    ...
    for iel in range(0, nel):
        vtufile.write(
            ...
            % (
                ...
                + ls_rho_b[iel] * (x[icon[0, iel]] - x_e[iel])
                + ls_rho_c[iel] * (z[icon[0, iel]] - z_e[iel])
                + ls_rho_d[iel] * (x[icon[0, iel]] - x_e[iel]) * (z[icon[0, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_rho_b[iel] * (x[icon[1, iel]] - x_e[iel])
                + ls_rho_c[iel] * (z[icon[1, iel]] - z_e[iel])
                + ls_rho_d[iel] * (x[icon[1, iel]] - x_e[iel]) * (z[icon[1, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_rho_b[iel] * (x[icon[2, iel]] - x_e[iel])
                + ls_rho_c[iel] * (z[icon[2, iel]] - z_e[iel])
                + ls_rho_d[iel] * (x[icon[2, iel]] - x_e[iel]) * (z[icon[2, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_rho_b[iel] * (x[icon[3, iel]] - x_e[iel])
                + ls_rho_c[iel] * (z[icon[3, iel]] - z_e[iel])
                + ls_rho_d[iel] * (x[icon[3, iel]] - x_e[iel]) * (z[icon[3, iel]] - z_e[iel])
            )
        )
    vtufile.write("</DataArray>\n")
    # --
    ...
    for iel in range(0, nel):
        vtufile.write(
            ...
            % (
                ...
                + ls_eta_b[iel] * (x[icon[0, iel]] - x_e[iel])
                + ls_eta_c[iel] * (z[icon[0, iel]] - z_e[iel])
                + ls_eta_d[iel] * (x[icon[0, iel]] - x_e[iel]) * (z[icon[0, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_eta_b[iel] * (x[icon[1, iel]] - x_e[iel])
                + ls_eta_c[iel] * (z[icon[1, iel]] - z_e[iel])
                + ls_eta_d[iel] * (x[icon[1, iel]] - x_e[iel]) * (z[icon[1, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_eta_b[iel] * (x[icon[2, iel]] - x_e[iel])
                + ls_eta_c[iel] * (z[icon[2, iel]] - z_e[iel])
                + ls_eta_d[iel] * (x[icon[2, iel]] - x_e[iel]) * (z[icon[2, iel]] - z_e[iel])
            )
        )
        vtufile.write(
            "%e\n"
            % (
                ...
                + ls_eta_b[iel] * (x[icon[3, iel]] - x_e[iel])
                + ls_eta_c[iel] * (z[icon[3, iel]] - z_e[iel])
                + ls_eta_d[iel] * (x[icon[3, iel]] - x_e[iel]) * (z[icon[3, iel]] - z_e[iel])
            )
        )
    vtufile.write("</DataArray>\n")
    # --
...
###################################################################################################
# particle population control function
###################################################################################################

# should I restrict it to boundary elements ?
...
def population_control(
    ...
    #################################################################
    # how many are needed because of elements below threshold?

    nparticle_needed = 0
    for iel in range(0, nel):
        nparticle_needed += max(0, nparticle_min - nparticle_e[iel])

    print("     -> number of needed particles", nparticle_needed)

    if nparticle_needed == 0:
        print("     -> no population control needed")
        return

    #################################################################
    # how many particles are available to be reused?

    nparticle_available = nparticle - np.sum(swarm_active)

    print("     -> number of inactive/available particles", nparticle_available)

    ptcl_active_file.write("%d %d %d %d\n" % (istep, np.sum(swarm_active), nparticle_needed, nparticle_available))
    ...
    for ip in range(0, nparticle):
        if not swarm_active[ip]:
            list_available_particles[counter] = ip
            counter += 1

    # print(list_available_particles)

    ##############################################################
    # compute number of elements that need population control

    nel_flagged = np.sum(nparticle_e < nparticle_min)

    print("     -> number of flagged elements", nel_flagged)

    ##############################################################
    # make list of flagged elements that need particles added to them

    list_flagged_elements = np.zeros(nel_flagged, dtype=np.int32)
    counter = 0
    for iel in range(0, nel):
        if nparticle_e[iel] < nparticle_min:
            list_flagged_elements[counter] = iel
            counter += 1

    ##############################################################
    # assess how many particles need to be added to each flagged element
    ...
    for k in range(nel_flagged):
        ...
        for ip in range(nb_particles_to_be_added_to_elt[iel]):
            ...
            swarm_z[particle_id] = np.dot(N[:], z_V[icon_V[:, iel]])
            ...
            swarm_eta[particle_id] = np.average(swarm_eta[mask])
            swarm_u[particle_id] = np.average(swarm_u[mask])
            swarm_w[particle_id] = np.average(swarm_w[mask])
        ...
        np.savetxt(
            ...
            np.array(
                [
                    ...
                    swarm_r[swarm_active],
                    swarm_t[swarm_active],
                    swarm_iel[swarm_active],
                ]
            ).T,
        )
