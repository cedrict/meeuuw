...
@numba.njit
def compute_pressure_offset(
    geometry: str,
    pressure_normalisation: str,
    axisymmetric: bool,
    top_element,
    nel,
    nelx,
    ...
    match pressure_normalisation:
            ...
            if axisymmetric:
                ...
                for iel in range(0, nel):
                    for iq in range(0, nq_per_element):
                        pressure_offset += (
                            np.dot(N_P[iq, :], p[icon_P[:, iel]]) * JxWq[iel, iq] * 2 * np.pi * xq[iel, iq]
                        )
            else:
                pressure_offset = 0
                for iel in range(0, nel):
        ...
        case _:
            raise ValueError("compute_pressure_offset: unknown pressure_normalisation value")
