...
is one more in my collection: SoPHa, SPHene, FANTOM, ELEFANT, FLAPS, FieldStone\footnote{\url{https://cedrict.github.io/}}.

%==============================================================================
\section{Install process}

The code consists of about two dozens of Python files. 
Aside from standard Python modules (numpy, scipy, etc ...) the 
numba\footnote{\url{https://numba.pydata.org/}} package also needs to be installed on the
...
There is no adaptive mesh refinement nor mesh 'stretching'. 
...
are rather mild with respect with the amount of RAM even laptops now have.
...
\item \lstinline{'least_squares'} -- see subsection below 
\end{itemize}
...
Remark: One could consider the above least-square approach with $\tilde{f}=a$, i.e. $\tilde{f}$ is
