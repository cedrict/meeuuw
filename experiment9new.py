...
def particle_layout(nparticle, swarm_x, swarm_z, swarm_rad, swarm_theta, Lx, Lz):
    ...
    for ip in range(nparticle):
        ...
        if (
            abs(swarm_theta[ip] - np.pi / 2) < np.pi * 0.2135
...
def material_model(
    ...
    swarm_rho[mask] = rho_crust  # -3050
    mask = swarm_mat == 2
    swarm_eta[mask] = eta_lithosphere
    swarm_rho[mask] = rho_lithosphere  # -3050
    mask = swarm_mat == 3
    swarm_eta[mask] = eta_uppermantle
    swarm_rho[mask] = rho_uppermantle  # -3050
    mask = swarm_mat == 4
    swarm_eta[mask] = eta_lowermantle
    swarm_rho[mask] = rho_lowermantle  # -3050
    mask = swarm_mat == 5
    swarm_eta[mask] = eta_blob
    swarm_rho[mask] = rho_blob  # -3050
