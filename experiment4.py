...
use_melting = True
adiabatic_surface_temperature = 1700.0  # K
...
if geometry == "quarter":
    print(
        "Ra=",
        (Tbottom - Ttop) * rho0 * 9.81 * alpha * (Router - Rinner) ** 3 / eta0 / (hcond0 / hcapa0 / rho0),
    )
...
def adiabatic_temperature(
    Tpotential, alpha, g, hcapa, d, top_tbl_thickness, bot_tbl_thickness, Ttop, Tbottom, total_depth
):

    dA = top_tbl_thickness
    TA = Tpotential * np.exp(alpha * g * dA / hcapa)
    dB = total_depth - bot_tbl_thickness
    TB = Tpotential * np.exp(alpha * g * dB / hcapa)
