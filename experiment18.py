...
def particle_layout(nparticle,swarm_x,swarm_z,swarm_rad,swarm_theta,Lx,Lz):
    ...
    for ip in range(0,nparticle):
        if swarm_z[ip]<0.75: swarm_mat[ip]=2 # asthenosphere 
        if (swarm_x[ip]-0.5)**2+(swarm_z[ip]-0.6)**2<0.123456789**2 : swarm_mat[ip]=3 # sphere
...
def material_model(nparticle,swarm_mat,swarm_x,swarm_z,swarm_rad,swarm_theta,\
    ...
    mask=(swarm_mat==1) ; swarm_eta[mask]=1e-3 ; swarm_rho[mask]=0  #air
    mask=(swarm_mat==2) ; swarm_eta[mask]=1    ; swarm_rho[mask]=1  #asthenosphere
    mask=(swarm_mat==3) ; swarm_eta[mask]=1e3  ; swarm_rho[mask]=2  #sphere
