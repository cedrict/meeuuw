...
from basis_functions import basis_functions_V_dr, basis_functions_V_dt
...
def compute_elemental_strain_rate(icon_V, u, v, nn_V, nel, x_V, y_V):
    ...
    for iel in range(0, nel):
        ...
        exy[iel] = np.dot(dNdx, v[icon_V[:, iel]]) * 0.5 + np.dot(dNdy, u[icon_V[:, iel]]) * 0.5
...
def compute_nodal_strain_rate(icon_V, u, v, nn_V, m_V, nel, dNdr_V_n, dNds_V_n, jcbi00n, jcbi01n, jcbi10n, jcbi11n):
    ...
    for iel in range(0, nel):
        for i in range(0, m_V):
            ...
            exy_n[inode] += 0.5 * np.dot(dNdx, v[icon_V[:, iel]]) + 0.5 * np.dot(dNdy, u[icon_V[:, iel]])
            count[inode] += 1
        # end for
    # end for
    exx_n /= count
...
##@numba.njit
def compute_nodal_strain_rate2(
    bignb,
    II,
    JJ,
    m_T,
    ...
    for iel in range(0, nel):
        ...
        for iq in range(0, nq_per_element):
            ...
            exyq = 0.5 * np.dot(dNdx, v[icon_V[:, iel]]) + 0.5 * np.dot(dNdy, u[icon_V[:, iel]])
